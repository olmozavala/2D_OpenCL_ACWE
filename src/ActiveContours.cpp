 ...
 */
void ActiveContours::initImagesArraysAndBuffers(GLuint& tbo_in, GLuint& tbo_out, bool initBuffers) {
    ...
    try {
        ...
        //Trying to clean the previous textures being used
        if(!initBuffers){
            cl_textures.erase(cl_textures.begin());
            cl_textures.erase(cl_textures.begin());
            //Cleaning previous arrays
            //            delete[] arr_img_phi;
            //            delete[] arr_buf_mask;
            //            delete[] arr_max_F;
...
/**
 * Iterates the Active Contours without Edges segmentation algorithm.
 * @param {int} The number of iterations that will be executed
 * @param {bool} If we are using all the bands in the image or not
 */
    ...
    try {
        ...
        if (usingOGL) {
            ...
            if (currIter == 0) {
                ...
                dout << "After buf_sdf-> img_phi" << endl;
...
/**
 * Smooths the new phi by forcing the gradient of the surface to be close to one.
 * It is using the Sussman smoothing function.
 * @param {vector<cl::Event>} The events that this kernel depends on 
 * @param {float} Alpha is dt in the function of sussman
 */
...
void ActiveContours::createRGBAMask(int width, int height, int xstart, int xend,
    ...
    dout << "--------------------- Creating mask (" << width << "," << height << ") ----------" << endl;
